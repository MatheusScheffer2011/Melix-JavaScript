...
collector.on('collect', m => {
  ...
  if (prefix.length > 4) {
    ...
    return msg.channel.send(`prefix tem que ser maior!!!`)
  } else {
	message.channel.send(`Você tem certeza que deseja auterar o prefix para: ${m.content}`).then(msg => {
   msg.react('✅');
   msg.react('❌')
    ...
    sim.on(`collect`, r => {
     ...
     return msg.channel.send(`Prefixo auterado para: ${m.content}`)
    })
    não.on(`collect`, r => {
